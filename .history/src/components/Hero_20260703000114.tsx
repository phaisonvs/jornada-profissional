...
const stats: StatItem[] = [
  ...
  },
  {
    ...
    popup: {
      ...
      summary:
        "Atuei em +50 entregas que exigiam conexão entre experiência, tecnologia, regra de negócio e operação.",
      deliveries: [
  ...
  },
  {
    ...
    popup: {
      ...
      summary:
        "Atuei diretamente na mensuração dos funis para transformar comportamento do usuário em decisão prática de CRO.",
      deliveries: [
        "Revisei eventos em login, checkout, formulários, leads e conversão.",
        "Ajustei dataLayer, parâmetros, regras de disparo e leitura em analytics.",
        "Validei a conexão entre interface, implementação e mensuração.",
        "Usei essa leitura para identificar gargalos e priorizar melhorias.",
      ],
    },
  },
  {
    ...
    valueFormat: (v) => `+${Math.round(v[0])}`,
    supportLine:
      "LPs e fluxos publicados em diferentes ambientes digitais.",
    popup: {
      title: "Jornadas e LPs publicadas",
      summary:
        "Atuei em jornadas publicadas em diferentes ambientes, conectando campanha, captação, front-end e operação.",
      deliveries: [
        "Publiquei LPs e fluxos em Salesforce, Wake, Sharp e domínios próprios.",
        "Estruturei formulários, CTAs, WhatsApp, captação de leads e fluxos comerciais.",
        "Adaptei front-end, responsividade e interações conforme cada plataforma.",
        "Garanti que a jornada estivesse pronta para campanha, operação e conversão.",
      ],
    },
  },
  {
    ...
    popup: {
      ...
      deliveries: [
        ...
        "Evoluí interações do cliente diante de PDPs e checkout.",
...
const Hero = () => {
  ...
  const typingFrameRef = useRef<Map<number, number>>(new Map());
  ...
  useEffect(() => {
    ...
    if ("typingLabel" in stat && stat.typingLabel) return;
    const targets =
  ...
  useEffect(() => {
    ...
    const rafMap = typingFrameRef.current;
    const existing = rafMap.get(cardIndex);
    ...
    const target = stat.typingLabel;
    const start = performance.now();
    const msPerChar = 110;

    const run = () => {
      const elapsed = performance.now() - start;
      const chars = Math.min(target.length, Math.floor(elapsed / msPerChar) + 1);
      setTypedValues((prev) => {
        const copy = [...prev];
        copy[cardIndex] = target.slice(0, chars);
        return copy;
      });
    ...
    return () => {
      const current = rafMap.get(cardIndex);
